...
| `Makefile` | `make ship M="..."`, and `package`, `lint`, `check` underneath it |
...
URL for a nested `TemplateURL` and Git sync has no packaging step, so the file
it deploys has to carry real bucket URLs. One command does all of it:

```bash
make ship M="split security out of network"
```

That lints, packages, commits and pushes — packaging is inside the command you
ship with, so it cannot be skipped. `make package` on its own is there for when
you want to see the diff first.

Skip it and nothing happens: `main.packaged.yaml` is unchanged, Git sync sees no
change, and your edit is not deployed. CI runs `make check` as a backstop and
fails the build when the committed copy is stale. Nothing is generated by CI and
