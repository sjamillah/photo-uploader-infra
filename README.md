...
|---|---|---|
| `photo-app-bootstrap` | created by hand once, then Git sync from `deployments/bootstrap.yaml` | the four IAM roles, the OIDC provider, ECR, the template staging bucket |
...
| File | Holds |
|---|---|
| `bootstrap.yaml` | Deployed by hand: IAM roles, OIDC provider, ECR, staging bucket |
| `main.yaml` | The parent, referencing the ten templates below by path |
| `main.packaged.yaml` | What Git sync deploys. Generated by `make package`, committed |
| `Makefile` | `make package`, `make lint`, `make check` |
...
| `templates/network.yaml` | VPC, subnets, routing, VPC endpoints, security groups |
...
| `templates/service.yaml` | Load balancer, ECS cluster and service, auto scaling, CodeDeploy |
| `templates/codedeploy.yaml` | The blue/green application and the two alarms that gate a cutover |
| `templates/pipeline.yaml` | EventBridge rules, CodePipeline, artifact bucket |
| `templates/monitoring.yaml` | Alarms that notify, not ones that block |

### Changing a template

`main.yaml` references its children by path. CloudFormation will not accept a
relative `TemplateURL` and Git sync has no packaging step, so the file it
deploys carries real bucket URLs:

```bash
make package        # uploads the children, writes main.packaged.yaml
git add main.yaml templates/ main.packaged.yaml
```

Commit the packaged file with the change that caused it. CI runs `make check`,
which packages again and fails if the committed copy is stale — so a forgotten
run is caught at review, not at deploy. Nothing is generated by CI and nothing
is committed by a bot.

Deploy order is `ecr -> alerts -> network -> media -> database -> iam ->
service -> codedeploy -> pipeline -> monitoring`, which CloudFormation works out
from the `!GetAtt` references between them.
...
| All resources via CloudFormation Git sync | `bootstrap.yaml`, `templates/` | Git sync tab showing a synced commit on **both** stacks; the main stack's Resources tab listing ten nested children |
